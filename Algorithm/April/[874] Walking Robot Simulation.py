...
def move(obstacles, head,com, (x,y)):

    if head[0] == 'N': 
        while (x,y+1) not in obstacles and com != 0 : 
            y += 1 
            com -=1 
    elif head[0] == 'E':
        while (x+1,y) not in obstacles and com != 0 : 
            x += 1 
            com -=1 
...
def robotSim(commands, obstacles):
    ...
    """
    (x,y) = (0,0)
    head , obstacles = deque(['N','E','S','W']) , set(map(tuple, obstacles))
    
    maxval = 0
    for com in commands: 
        if com == -1 : head.rotate(-1)
        elif com == -2: head.rotate(1)
        else: 
            (x,y)= move(obstacles, head,com,(x,y))
            maxval = max(maxval, x** 2 + y ** 2)
        print((x,y))
